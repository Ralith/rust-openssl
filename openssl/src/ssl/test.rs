...
fn stateless() {
    ...
    server_ctx.set_cookie_generate_cb(|_tls, buf| { buf[0..COOKIE.len()].copy_from_slice(COOKIE); Ok(COOKIE.len()) });
    server_ctx.set_cookie_verify_cb(|_tls, buf| buf == COOKIE);
